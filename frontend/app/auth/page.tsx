...
export default function AuthPage() {
  ...
  const showMessage = (text: string, type: MsgType = 'success') => {
    setMessage({ text, type });
    setTimeout(() => setMessage({ text: '', type: 'success' }), 4000);
  };

  // If this is on a <form onSubmit={...}>
  const handleSignIn = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setLoading(true);
    try {
      // TODO: your real sign-in logic
    ...
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden">
      <div className="absolute inset-0">
        <div
          style={{
            background:
              'linear-gradient(135deg, #f5d5e3, #e8b4d3, #d896c4, #c47ab5)',
            opacity: 0.8,
          }}
      ...
      <div className="relative z-10 max-w-md w-full mx-auto px-6">
        ...
        <form onSubmit={isSignup ? handleSignUp : handleSignIn} className="flex flex-col gap-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="bg-[#a4556f] text-white py-2 rounded-lg font-bold hover:bg-[#8b4558]"
          >
